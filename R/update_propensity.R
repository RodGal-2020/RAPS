...
update_propensity = function(rap, verbose = 1, debug = FALSE) {
  ...
  ##############################################################################
  get_stochastic_constant = function(chosen_rule) {
    ## Debugging
    # chosen_rule = rules[1, ]
    lhs_object = chosen_rule$lhs[[1]] %>%
    ...
    if (n_objects_lhs == 1 &&
      ...
      return(chosen_rule$kinetic_constant)

    } else if (n_objects_lhs == 1 &&
               lhs_object$multiplicity == 2) {
      return(2 * chosen_rule$kinetic_constant / volume)

    } else if (n_objects_lhs > 1 &&
               all(lhs_object$multiplicity == 1)) {
      return(chosen_rule$kinetic_constant / volume)
  ...
  if (! "stochastic_constant" %in% colnames(rules)) {
    rules %<>% dplyr::mutate(
        stochastic_constant = get_stochastic_constant(chosen_rule),
        .keep = "all"
      )
  }
